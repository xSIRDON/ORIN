...
public class UnbanCommand implements CommandExecutor {
    ...
    public UnbanCommand(JavaPlugin plugin, RankManager rankManager, NameManager nameManager, PermissionsManager permissionsManager, PunishmentManager punishmentManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        // ✅ Ensure the player is actually banned
        if (!punishmentManager.isBanned(targetUUID) && !punishmentManager.isTempBanned(targetUUID)) {
            sender.sendMessage(ChatUtil.darkRed + "❌ " + rankPrefix + targetDisplayName + ChatUtil.red + " is not banned.");
            return true;
        }

        // ✅ Unban the player
        String staffName = (sender instanceof Player) ? sender.getName() : "SERVER";
        punishmentManager.unbanPlayer(targetName, staffName);

        // ✅ Announce unban
        sender.sendMessage(ChatUtil.green + "✔ " + rankPrefix + targetDisplayName +
                ChatUtil.white + " has been unbanned.");
